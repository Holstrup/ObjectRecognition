...
def reinitialize_table():
    ...
    cur = con.cursor()
    cur.execute("DROP TABLE IF EXISTS encoding_table")
    cur.execute("CREATE TABLE encoding_table (encoding ARRAY, label VARCHAR(10))")
...
def add_encoding(classification, label="Null"):
    ...
    if label == "Null":
        query = "INSERT INTO encoding_table(encoding, label) VALUES(?, NULL)"
        cursor.execute(query, (classification,))
    else:
        query = "INSERT INTO encoding_table(encoding, label) VALUES(?, ?)"
...
def get_known_encodings():
    ...
    :return: encodings array of (128, n)
             labels list of (n)
    """
    query = "SELECT * FROM encoding_table WHERE label IS NOT NULL"
    cursor, connection = connect()
